"use client";

import { useConsent } from "@/components/analytics/ConsentProvider";

const AD_PROVIDERS = [
  process.env.NEXT_PUBLIC_META_PIXEL_ID?.trim() ? "Meta (Pixel und Conversions API)" : null,
  process.env.NEXT_PUBLIC_GA4_ID?.trim() || process.env.NEXT_PUBLIC_GOOGLE_ADS_ID?.trim()
    ? "Google (Ads/Analytics)" : null,
].filter(Boolean);

export function ConsentBanner() {
  const { decision, setDecision, analyticsEnabled } = useConsent();

  if (!(AD_PROVIDERS.length || analyticsEnabled) || decision !== null) {
    return null;
  }

  return (
    <div
      role="dialog"
      aria-live="polite"
      aria-label="Hinweis zu Cookies und Reichweitenmessung"
      className="fixed inset-x-0 bottom-0 z-[9999] border-t border-mh-text-on-dark/10 bg-mh-ink-950 px-mh-4 py-mh-4 text-mh-text-on-dark shadow-[0_-8px_30px_rgba(0,0,0,0.35)] backdrop-blur-sm"
    >
      <div className="mx-auto flex max-w-[1080px] flex-col gap-mh-4 sm:flex-row sm:items-center sm:justify-between">
        <p className="max-w-[64ch] mh-body-small text-mh-text-on-dark/85">
          {analyticsEnabled ? "Mit Ihrer Zustimmung messen wir Seitenaufrufe und Kontaktaktionen mit Vercel Web Analytics. " : ""}
          {AD_PROVIDERS.length ? `Nach Zustimmung nutzen wir außerdem ${AD_PROVIDERS.join(" und ")} zur Werbe- und Reichweitenmessung. ` : ""}
          Die Zustimmung ist freiwillig; Daten können in den USA verarbeitet werden.
          Mehr in der{" "}
          <a
            href="/datenschutz"
            className="font-semibold text-mh-glow underline underline-offset-2 hover:text-mh-text-on-dark"
          >
            Datenschutzerklärung
          </a>
          . Widerruf jederzeit möglich.
        </p>
        <div className="flex shrink-0 gap-mh-3">
          <button
            type="button"
            onClick={() => setDecision("denied")}
            className="min-h-[44px] rounded-mh-pill border border-mh-text-on-dark/35 bg-transparent px-mh-5 font-mh-body text-[15px] font-semibold text-mh-text-on-dark transition-colors hover:bg-mh-text-on-dark/10"
          >
            Nur notwendige
          </button>
          <button
            type="button"
            onClick={() => setDecision("granted")}
            className="min-h-[44px] rounded-mh-pill border-none bg-mh-glow px-mh-6 font-mh-body text-[15px] font-bold text-mh-ink-950 transition-colors hover:bg-mh-text-on-dark"
          >
            Akzeptieren
          </button>
        </div>
      </div>
    </div>
  );
}
