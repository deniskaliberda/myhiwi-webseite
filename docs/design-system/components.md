...
## Statistics reference page (12.09.2026)

`app/wissen/digitalisierung-ki-statistiken/` uses existing Section, Container, SectionMark and CTA atoms. Editorial hero; linked short findings; study comparison; source-grouped statistic rows with stable anchors, period and population; two static horizontal bar charts with equivalent HTML values; source register and CSV export; one final visibility CTA. All values derive from `content/research/digitalisierung-ki.json`. Blue/ink/paper tokens, Bricolage headings and existing reading width. Tables scroll within a labelled region on mobile; rows and charts stack. No animation, new font, tracking, filters, or client-side chart library. Exported figures use the same palette and explicitly credit original studies; MyHiwi is the editor, not the data producer.
